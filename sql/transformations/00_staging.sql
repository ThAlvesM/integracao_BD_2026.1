...
-- Deve ser criada antes de todas as outras views.

CREATE OR REPLACE VIEW elt.vw_staging AS
SELECT
    ...
    -- ── GESTÃO (descrição via CASE, espelhando mapa_gestao do ETL) ────────
    ...
    CASE LOWER(TRIM(tp_gestao::TEXT))
        ...
        WHEN 'd' THEN 'dupla'
        ELSE NULL          -- valores fora do domínio ficam NULL, como no .map() do pandas
    ...
    -- mapa_meses do ETL traduzido para SQL
...
-- motivo_desabilitacao é descartada aqui (drop() do ETL)
