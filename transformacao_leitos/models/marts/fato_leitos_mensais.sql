...
FROM {{ ref('vw_staging') }} s

LEFT JOIN {{ ref('dim_tempo') }} t
    ON s.competencia = t.competencia

LEFT JOIN {{ ref('dim_estabelecimento_saude') }} e
    ON  s.cnes                 = e.cnes
